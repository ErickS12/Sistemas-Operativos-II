...
public class Kernel extends Thread
{
  ...
  //declaracion de clases y variables
  private static int virtPageNum = 63;//numero de paginas
  ...
  private ControlPanel controlPanel ;//Parte grafica
  private Vector memVector = new Vector();// vector de paginas
  ...
  public long block = (int) Math.pow(2,12); //tamaño
  public static byte addressradix = 10;

  public void init( String commands , String config )  //inicializa TODO
  {
    ...
    //// Dentro de init, después de calcular el block
...
public void step()
  {
    ...
    // Obtenemos el numero de pagina (P0, P1, etc.)
    int pageId = Virtual2Physical.pageNum( instruct.addr , virtPageNum , block );
    getPage( pageId );

    // --- CÁLCULO DE SEGMENTOS (NUEVO) ---
    // 1. Definimos el tamaño de un segmento (1/4 de la página)
    long segmentSize = block / 4; 
    
    // 2. Calculamos el offset (posición dentro de la página)
    long pageOffset = instruct.addr % block; 
    
    // 3. Obtenemos el ID del segmento (0, 1, 2 o 3)
    int segmentID = (int) (pageOffset / segmentSize);
    // ------------------------------------
    ...
    {
      Page page = ( Page ) memVector.elementAt( pageId );
      if ( page.physical == -1 ) 
      {
        if ( doFileLog )
        {
        ...
        // Al traer la página, marcamos el segmento que causó el fallo
        page.segR[segmentID] = 1; // <--- NUEVO: Marcar segmento tras Page Fault
      } 
      else 
      {
        page.R = 1;
        page.segR[segmentID] = 1; // <--- NUEVO: Marcar bit de referencia del segmento específico
        page.lastTouchTime = 0;   
        
        // Mensaje modificado para mostrar P y S
        String msg = "READ " + Long.toString( instruct.addr , addressradix ) + " ... okay (P" + pageId + ", S" + segmentID + ")";
    ...
    if ( instruct.inst.startsWith( "WRITE" ) ) 
    {
      ...
      if ( page.physical == -1 ) 
      {
        ...
        // Al traer la página, marcamos el segmento modificado
        page.segM[segmentID] = 1; // <--- NUEVO
      } 
      else 
      {
        page.M = 1;
        page.segM[segmentID] = 1; // <--- NUEVO: Marcar bit de modificación del segmento específico
        page.lastTouchTime = 0;

        // Mensaje modificado para mostrar P y S
        String msg = "WRITE " + Long.toString(instruct.addr , addressradix) + " ... okay (P" + pageId + ", S" + segmentID + ")";
    ...
    // Lógica de envejecimiento (Aging)
    for ( i = 0; i < virtPageNum; i++ ) 
    {
      Page page = ( Page ) memVector.elementAt( i );
      ...
      {
        page.R = 0;
        // Opcional: ¿Quieres resetear también los segmentos al envejecer?
        // Si sí, descomenta el bucle de abajo:
        /*
        for(int k=0; k<4; k++) {
            page.segR[k] = 0;
        }
